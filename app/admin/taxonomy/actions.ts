...
type Group = "contractor" | "project" | "equipment" | "material" | "business" | "rfq";
const groups = new Set<Group>(["contractor", "project", "equipment", "material", "business", "rfq"]);
...
function finish(message: string, error = false): never {
  ...
  ["/", "/add-listing", "/contractors", "/projects-tenders", "/equipment-marketplace", "/construction-materials", "/business-opportunities", "/rfqs", "/search", "/admin/taxonomy"].forEach((path) => revalidatePath(path));
...
export async function createTaxonomyItem(data: FormData) {
  ...
  try {
    ...
    else if (group === "rfq") await prisma.rfqCategoryOption.create({ data: { slug, name } });
    else {
...
export async function updateTaxonomyItem(data: FormData) {
  ...
  try {
    if (group === "contractor") await prisma.contractorType.update({ where: { slug }, data: { name } });
      ...
      const current = await prisma.rfqCategoryOption.findUnique({ where: { slug }, select: { name: true } });
      if (!current) finish("Category not found.", true);
      await prisma.$transaction([
        prisma.rfqCategoryOption.update({ where: { slug }, data: { name } }),
        prisma.rfq.updateMany({ where: { category: current.name }, data: { category: name } }),
      ]);
    }
  } catch {
...
export async function deleteTaxonomyItem(data: FormData) {
  ...
  if (group === "contractor") {
    ...
    const row = await prisma.rfqCategoryOption.findUnique({ where: { slug }, select: { name: true } });
    if (!row) finish("Category not found.", true); name = row.name; usage = await prisma.rfq.count({ where: { category: row.name } });
  }
  ...
  else if (group === "business") await prisma.businessCategoryOption.delete({ where: { slug } });
  else await prisma.rfqCategoryOption.delete({ where: { slug } });
