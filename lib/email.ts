...
export async function sendOtpEmail(
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD || !from) {
    ...
    console.info(`[GulfBuildHub development OTP] ${email}: ${code}`);
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: "Your GulfBuildHub verification code",
    text: `Your GulfBuildHub verification code is ${code}. It expires in 10 minutes. Never share this code with anyone.`,
...
export async function sendPasswordResetEmail(email: string, resetUrl: string) {
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: "Reset your GulfBuildHub password",
    text: `Reset your GulfBuildHub password using this link: ${resetUrl}. The link expires in 30 minutes. If you did not request it, ignore this email.`,
