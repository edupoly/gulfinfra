...
export function ProjectReview({
  ...
  if (state.success && state.slug) {
    return <section className="w-full rounded-[38px] border border-slate-200 bg-white px-5 py-10 shadow-[0_18px_60px_rgba(15,23,42,0.08)] sm:px-10 sm:py-14 lg:px-20"><Progress activeStep={4} complete /><div className="mx-auto mt-16 max-w-2xl rounded-3xl border border-emerald-200 bg-emerald-50 px-6 py-12 text-center"><div className="mx-auto grid size-16 place-items-center rounded-full bg-emerald-600 text-3xl font-black text-white">✓</div><p className="mt-6 text-sm font-black uppercase tracking-[0.2em] text-emerald-700">Project published</p><h1 className="mt-3 text-3xl font-black text-[#0b1f3a] sm:text-4xl">Your project listing is live</h1><p className="mt-4 text-lg text-slate-600">{state.message}</p><Link href={`/projects-tenders/${state.slug}`} className="mt-7 inline-flex rounded-full bg-[#0b1f3a] px-7 py-3 font-black text-white">View project details →</Link></div></section>;
  }
