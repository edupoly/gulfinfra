  ...
  return <section className="w-full rounded-[38px] border border-slate-200 bg-white px-5 py-10 shadow-[0_18px_60px_rgba(15,23,42,0.08)] sm:px-10 sm:py-14 lg:px-20">
    ...
    <form action={action} className="mx-auto mt-10 max-w-5xl space-y-8">
      ...
      <Box title="Project images" description="Add up to six project renderings, site images, or plans.">
        {[1, 2, 3].map((item) => <label key={item}><b>Image {item} URL</b><input name="imageUrls" type="url" defaultValue={item === 1 ? "https://images.unsplash.com/photo-1541888946425-d81bb19240f5" : ""} placeholder="https://..." className={field} /></label>)}
        {state.errors?.images && <p className="font-bold text-red-600">{state.errors.images}</p>}
      </Box>
      <Box title="Tender documents" description="Add up to five documents.">
        {[1, 2].map((item) => <div key={item} className="grid gap-4 rounded-2xl border border-slate-200 p-4 sm:grid-cols-[1fr_190px]"><label><b>Document name</b><input name="documentNames" defaultValue={item === 1 ? "Tender Scope of Works" : ""} className={field} /></label><label><b>Type</b><select name="documentTypes" defaultValue="Tender Document" className={field}><option>Tender Document</option><option>Drawing</option><option>Bill of Quantities</option><option>Specification</option><option>Other</option></select></label><label className="sm:col-span-2"><b>Document URL</b><input name="documentUrls" type="url" defaultValue={item === 1 ? "https://example.com/riyadh-metro-scope.pdf" : ""} className={field} /></label></div>)}
