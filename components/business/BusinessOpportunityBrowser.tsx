...
export function BusinessOpportunityBrowser({ opportunities, initialSearch }: { opportunities: BusinessOpportunity[]; initialSearch: string }) {
    ...
    <section>
      ...
      <div className="space-y-4">
        {results.map((item) => <article key={item.slug} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md md:grid md:grid-cols-[170px_minmax(0,1fr)_190px]">
          ...
          <div className="flex flex-col justify-between border-t border-slate-200 p-5 md:border-l md:border-t-0"><p className="text-right text-xs text-slate-500">Posted {item.postedDate}</p><div className="mt-5 space-y-2"><a href={`mailto:${item.contact}?subject=${encodeURIComponent(item.title)}`} className="block rounded-full bg-slate-950 px-4 py-2 text-center text-sm font-bold text-white">Contact</a><div className="grid grid-cols-2 gap-2"><a href={`tel:${item.phone.replace(/\\s/g, "")}`} className="rounded-full border px-3 py-2 text-center text-xs font-bold">Call</a><a href={`https://wa.me/${item.whatsapp.replace(/\\D/g, "")}`} className="rounded-full bg-emerald-50 px-3 py-2 text-center text-xs font-bold text-emerald-700">WhatsApp</a></div></div></div>
        </article>)}
